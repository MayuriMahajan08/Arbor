...
-- salary: ₹70,000, DOJ: 2024-01-01) and Aarav Jain (email: aarav.jain@example.com, salary:₹72,000, DOJ: 2024-01-02), in the IT department (DepartmentID: 3). 
-- Add these records.
...
-- Situation -2
-- The Marketing department has been created (DepartmentID: 11) with a budget of ₹13,00,000, located in Pune, and headed by Rakesh Mehta. 
-- Add this department.

Insert INTO Departments(DepartmentName, Location, HeadOfDepartment, AnnualBudget) value
...
Insert INTO Employees (FirstName, LastName, DepartmentID, Salary, DateOfJoining, Email) value
...
-- Arjun Shah (email: arjun.shah@example.com, salary: ₹75,000, DOJ: 2024-04-01) and 
...
-- Situation -9
-- The Logistics department (DepartmentID: 13) has been created with a budget of ₹9,00,000, located in Chennai, and headed by Ravi Verma.
-- Add this department.
...
-- Kavita Desai (email: kavita.desai@example.com, salary: ₹50,000, DOJ: 2024-05-01) and 
...
Select * from Employees 
where DateOfJoining = '2024-02-01' AND '2024-03-01';
or
Select * from Employees 
where DateOfJoining = '2024-02-01';
...
select * from employees 
where firstname like '%manager%'and lastname like '%manager%';
...
Select location, count(DepartmentName) 
from Departments
group by location;
...
select HeadOfDepartment, count(DepartmentName) 
from Departments
...
select location, min(AnnualBudget), Max(AnnualBudget) 
from Departments
group by location; 
...
Select concat('Hello', ' ',FirstName, ',') as personalized_email 
from employees; 

-- Situation -4: For creating a unique employee ID, concatenate the department ID and employe number.
Select concat(DepartmentID, ' ',EmployeeID) as unique_employeeID 
from employees;
...
Select concat(FirstName, ' ',year(DateOfJoining)) as Annual_Report 
from Employees;
...
-- Situation -3: The HR team has informed us of a change in the contact information of employee with EmployeeID 120. The new contact number for this employee should 
-- be updated to "9876543210". Ensure that the correct phone number is reflected in the system for any future communications.
update employees
set phone_number = '9876543210'
where employeeID =120;

-- Situation -4: The company has recently implemented a new email domain. All employees who joined after January 1, 2023, need their email addresses updated to 
-- reflect the new domain "@newcompany.com". Update the email addresses accordingly for all the affected employees.
update employees
set email = lower(concat(firstname, lastname, '@newcompany.com'))
where DateOfJoining < '2023-01-01'; 

-- Situation -5: It has come to our attention that there was an error in the naming of the HR department. The department name for DepartmentID 5 needs to be updated 
-- from "HR" to "Human Resources" to reflect the company's official title. Perform this update to ensure consistency across the records.
update departments 
...
-- Situation -7: The Marketing department has recently moved to a new office in a different city. The location of the Marketing department (DepartmentID: 11) needs 
-- to be updated to reflect this change. Please update the location to "Mumbai" in the department's records accordingly.
update departments 
set location = "Mumbai"
...
-- Situation -9: EmployeeID 110, who was previously working on a different project, has now been assigned to "Project Alpha" after the successful completion of 
-- their prior assignment. Please update the project assignment for this employee to reflect this new responsibility.

-- Situation -10: The employee with EmployeeID 103 had a delayed start date due to personal reasons. We need to update their joining date to the correct date, which 
-- is 2024-03-15, to ensure the records accurately reflect the employee's actual start date with the company.
update employees 
...
-- Situation -2: The company is undergoing a restructuring. As part of this process, we need to delete all employees in the HR department (DepartmentID: 2) 
...
-- Situation -5: After the closure of "Project Beta", we need to delete all records of employees who were working on this project 
...
-- Situation -7: The Marketing department (DepartmentID: 3) has undergone significant layoffs. Delete all records of employees who worked in the Marketing 
...
delete from employees
where email is null or contact_details is null; 

-- Situation -9: We need to remove all employees whose salary is above ₹1,00,000 but were part of a temporary contract that has now expired. Delete these 
-- employee records.
delete from employees
where salary > '100000' and contract_status = 'expired';

-- Situation -10: The company has decided to shut down operations in certain regions. Delete the employee records of those who were hired in 
-- the "North" region (assuming a Region column exists), and whose employment ended before 2023.
delete from employees
where Region ='North' and Employeement_end_date < '2023';
...
-- Situation -2: As the company has decided to start tracking employees' marital status, add a new column MaritalStatus of type VARCHAR(20) to the Employee table.
...
alter table employees
 ...
 -- the data type was decimal only, hence no changes made.

-- Situation -5: To better categorize employees, we need to add a new column EmployeeCategory in the Employee table. The column should accept values 
...
-- Situation -8: As part of a company-wide restructuring, the DepartmentID column in the Employee table should be renamed to TeamID to reflect the new 
-- team-based structure.
-- Note: Please retrieve the changes made for consistency of the data. If necessary, restore thecolumn name back to DepartmentID for future references.
alter table employees
...
-- Situation -9: The company has decided to archive old employee data, so the Employee table needs to be partitioned by JoinDate in order to keep 
-- the data for employees who joined before 2020 separate.
...
-- Situation -10: We need to create a foreign key constraint between the Employee table and the Department table to ensure the integrity of the department 
-- data linked to each employee.
